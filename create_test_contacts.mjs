...
// Configura il client DB per Neon Postgres
...
// Funzione per selezionare casualmente alcune aziende dal database
async function getRandomCompanies(count) {
  try {
...
async function createTestContacts(count) {
  try {
    ...
    // Ottieni alcune aziende per associarle ai contatti
    const companies = await getRandomCompanies(10);
    console.log(`Trovate ${companies.length} aziende da associare ai contatti`);
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Inserisci il contatto nel database usando l'API REST
      try {
    ...
    console.error('Errore durante la creazione dei contatti di test:', error);
  } finally {
