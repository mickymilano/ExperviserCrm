...
export async function getContactsByCompany(companyId: number): Promise<Contact[]> {
  try {
    console.log(
      "getContactsByCompany: retrieving contacts for company", companyId
    );
...
export async function getUnassignedContacts(): Promise<Contact[]> {
  try {
    console.log(
      "getUnassignedContacts: retrieving contacts without company"
    );
...
export async function updateContactCompany(contactId: number, companyId: number | null): Promise<boolean> {
  try {
    console.log(
      `updateContactCompany: updating contact ${contactId} with company ${companyId || 'NULL'}`
    );
