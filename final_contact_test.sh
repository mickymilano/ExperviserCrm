...
# Verifica direttamente le aree di attività nel database
echo "Verifico aree di attività nel database:"
AREAS=$(psql $DATABASE_URL -c "SELECT * FROM areas_of_activity WHERE contact_id = $CONTACT_ID;")
echo "$AREAS"
