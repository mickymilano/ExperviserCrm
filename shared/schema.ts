...
/**
 * EMAIL ACCOUNTS
 * Tabella degli account email configurati
 */
export const emailAccounts = pgTable('email_accounts', {
  id: serial('id').primaryKey(),
  userId: integer('user_id').notNull().references(() => users.id),
  name: varchar('name', { length: 100 }).notNull(),
  email: varchar('email', { length: 100 }).notNull(),
  imapHost: varchar('imap_host', { length: 100 }).notNull(),
  imapPort: integer('imap_port').notNull(),
  imapSecurity: varchar('imap_security', { length: 20 }).notNull(), // 'ssl', 'tls', 'none'
  smtpHost: varchar('smtp_host', { length: 100 }).notNull(),
  smtpPort: integer('smtp_port').notNull(),
  smtpSecurity: varchar('smtp_security', { length: 20 }).notNull(), // 'ssl', 'tls', 'none'
...
/**
 * EMAILS
 * Tabella delle email ricevute e inviate
 */
export const emails = pgTable('emails', {
  id: serial('id').primaryKey(),
  userId: integer('user_id').notNull().references(() => users.id),
  ...
  entityType: varchar('entity_type', { length: 50 }), // 'contact', 'company', 'deal', 'lead'
...
/**
 * EMAIL ATTACHMENTS
 * Tabella degli allegati delle email
 */
